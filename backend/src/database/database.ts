...
const usersField: string =
  "id SERIAL PRIMARY KEY, username VARCHAR(32), email VARCHAR(32), password VARCHAR(255), firstname VARCHAR(32), lastname VARCHAR(32), gender VARCHAR(32), sex_pref VARCHAR(32), bio VARCHAR(500), hashtags VARCHAR(255)[10], age INTEGER, verified BOOLEAN DEFAULT FALSE, connection_status BOOLEAN DEFAULT FALSE, profile_picture VARCHAR(255)[5], latitude FLOAT DEFAULT 48, longitude FLOAT DEFAULT 2, fame_rating INTEGER DEFAULT 0, isfake BOOLEAN DEFAULT FALSE, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, CONSTRAINT unique_name UNIQUE(username), CONSTRAINT unique_email UNIQUE(email), CONSTRAINT profile_picture_length CHECK (array_length(profile_picture, 1) <= 5), CONSTRAINT hashtags_length CHECK (array_length(hashtags, 1) <= 10)";
